...
public abstract class AbstractSMA {
    ...
    public void run() {
        ...
        System.err.print("\t(stability: " + this.diagnostics.resultsIsStable() + ")\n");
...
//				System.out.println(p+" is married to "+p.getCurrentPartner());
    ...
    protected static void runAlgorithm(Class<? extends AbstractSMA> algorithmClass, String[] args) {
        try {
            if (args.length < 2) {
                System.err.println("I  need 2 input datasets: 1 for men and 1 for women (3rd argument is the step metrics)");
                System.exit(1);
            }
